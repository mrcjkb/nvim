...
with lib; let
  ...
  devPluginListType = types.submodule {
    options = with types; {
      directory = mkOption {
        type = nullOr str;
        description = "Where to checkout the dev plugins (relative to $HOME), if they aren't already checked out.";
        default = null;
        example = "git/devPlugins";
      };
      plugins = mkOption {
  ...
  customRC =
    ...
    ''
    + optionalString (cfg.devPlugins.plugins != [] && cfg.devPlugins.directory != null) (
      strings.concatMapStringsSep
      "\n"
      (plugin: ''
        dev_plugin_path = home .. '/${cfg.devPlugins.directory}/${plugin.name}'
        ...
        vim.opt.runtimepath:append(dev_plugin_path)
        vim.cmd.packadd('${plugin.name}')
      '')
    )
    cfg.devPlugins.plugins
