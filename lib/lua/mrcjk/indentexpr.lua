...
internal.avoid_force_reparsing = {
  yaml = true,
}

internal.comment_parsers = {
  comment = true,
  jsdoc = true,
  phpdoc = true,
}
...
---@return integer
function internal.get_indent(lnum)
  ...
  if not internal.avoid_force_reparsing[root_lang] then
  ...
  local root, lang_tree ---@type TSNode
  parser:for_each_tree(function(tstree, tree)
    if not tstree or internal.comment_parsers[tree:lang()] then
      return
    end
  ...
  while node do
    ...
    if should_process then
      ...
      is_in_err = parent ~= nil and parent:has_error()
    end
    if
      should_process
      and (
...
function M.indentexpr()
  return internal.get_indent(vim.v.lnum)
end
