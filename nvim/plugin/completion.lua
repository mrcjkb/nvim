...
vim.api.nvim_create_autocmd({ 'InsertEnter', 'CmdlineEnter' }, {
  once = true,
  group = vim.api.nvim_create_augroup('completion-setup', {}),
  callback = function()
    if vim.g.completion_setup_done then
      return
    end
    ...
    vim.iter(cmp_plugins):each(vim.cmd.packadd)

    -- HACK: cmp sources use after/plugin to register themselves
    -- This is an antipattern that we have to work around now.
    local dirs = vim.fs.joinpath('opt', 'cmp-')
    for _, file in ipairs(vim.api.nvim_get_runtime_file(vim.fs.joinpath('after', 'plugin', '*'), true)) do
      if file:match(dirs) then
        vim.cmd.source(file)
      end
    end
